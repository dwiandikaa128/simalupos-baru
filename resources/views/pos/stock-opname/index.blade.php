<x-layouts.pos :title="'Stock Opname'">
    <div class="p-6 max-w-6xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-display-sm font-bold text-on-surface">Stock Opname</h1>
                <p class="text-body-sm text-on-surface-variant">Input stok fisik dan cocokkan dengan sistem</p>
            </div>
        </div>
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="flex items-center gap-3">
                <a href="{{ route('pos.stock-opname.create') }}" class="flex items-center gap-2 px-5 py-2.5 bg-primary text-on-primary rounded-xl font-semibold text-body-sm min-h-[44px] hover:bg-primary-container transition-colors shadow-sm">
                    <span class="material-symbols-outlined text-[18px]">assignment</span>Mulai Opname Baru
                </a>
                @if($isFiltered)
                    <a href="{{ route('pos.stock-opname.index') }}" class="flex items-center gap-1 px-3 py-2 rounded-xl border border-outline-variant text-body-sm font-medium hover:bg-surface-dim transition-colors bg-white">
            ...
            {{-- Date Filter --}}
            <form method="GET" action="{{ route('pos.stock-opname.index') }}" class="flex flex-wrap items-end gap-3" x-data="{ mode: '{{ ($filterFrom && $filterTo) ? 'range' : 'single' }}' }">
                <div class="flex items-center gap-2 bg-surface rounded-xl border border-outline-variant px-3 py-1 bg-white">
        ...
        <div>
            ...
            <div class="flex flex-wrap gap-2">
                @foreach($opnameDates->take(10) as $dateItem)
                    ...
                    @endphp
                    <a href="{{ route('pos.stock-opname.index', ['date' => $dateItem->opname_date->format('Y-m-d')]) }}"
                        ...
                        <tr>
                            <td colspan="7" class="py-12 text-center text-on-surface-variant">
                                ...
                                @if($isFiltered)
                                    ...
                                    <a href="{{ route('pos.stock-opname.index') }}" class="text-primary-container text-label-sm mt-2 inline-block hover:underline">Lihat semua data</a>
                                @else

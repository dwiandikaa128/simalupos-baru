<x-layouts.admin :header="'Stock Opname Baru'" :subtitle="'Input stok fisik bahan yang dihitung'">

    <div class="mb-6">
        <a href="{{ route('admin.stock-opname.index') }}" class="inline-flex items-center gap-2 text-body-sm text-on-surface-variant hover:text-primary-container transition-colors">
            <span class="material-symbols-outlined text-[18px]">arrow_back</span>Kembali ke Riwayat
        </a>
    </div>

    <form method="POST" action="{{ route('admin.stock-opname.store') }}" id="opnameForm">
        @csrf
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.stock-opname.index') }}" class="px-5 py-2.5 rounded-xl border border-outline-variant text-body-sm font-medium hover:bg-surface-dim">Batal</a>

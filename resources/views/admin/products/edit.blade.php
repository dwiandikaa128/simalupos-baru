<x-layouts.admin :header="'Edit Produk'" :subtitle="$product->name">
    <div class="max-w-2xl">
        @if ($errors->any())
        ...
        <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-outline-variant p-6 space-y-5">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $product->name) }}" required class="w-full py-3 px-4 rounded-xl border border-outline-variant bg-surface text-body-md focus:border-primary-container focus:ring-1 focus:ring-primary-container/20">
                </div>
                <div>
                    <label class="block text-label-md font-semibold mb-2">Kategori</label>
                    <select name="category_id" required class="w-full py-3 px-4 rounded-xl border border-outline-variant bg-surface text-body-md">
                ...
                <div>
                    ...
                    <input type="number" name="base_price" value="{{ old('base_price', $product->base_price) }}" required class="w-full py-3 px-4 rounded-xl border border-outline-variant bg-surface text-body-md">
                </div>
                <div>
                    <label class="block text-label-md font-semibold mb-2">Deskripsi</label>
                    <textarea name="description" rows="3" class="w-full py-3 px-4 rounded-xl border border-outline-variant bg-surface text-body-md resize-none">{{ old('description', $product->description) }}</textarea>
                </div>
                <div>
                ...
                <div class="flex items-center gap-6">
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="is_available" value="1" {{ $product->is_available ? 'checked' : '' }} class="w-4 h-4 rounded border-outline-variant text-primary-container">
                        <span class="text-body-sm">Tersedia</span>
                    </label>
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="is_featured" value="1" {{ $product->is_featured ? 'checked' : '' }} class="w-4 h-4 rounded border-outline-variant text-primary-container">
            ...
            <div class="bg-white rounded-xl border border-outline-variant p-6">
                ...
                <div id="variants-container" class="space-y-3">
                    ...
                    <div class="flex items-center gap-3 variant-row">
                        ...
                        <input type="text" name="variants[{{ $i }}][name]" value="{{ $variant->name }}" class="flex-1 py-2.5 px-4 rounded-xl border border-outline-variant text-body-sm">
                        <input type="number" name="variants[{{ $i }}][price_modifier]" value="{{ $variant->price_modifier }}" class="w-40 py-2.5 px-4 rounded-xl border border-outline-variant text-body-sm">
            ...
            <div class="bg-white rounded-xl border border-outline-variant p-6">
                ...
                <p class="text-body-sm text-on-surface-variant mb-4">Pilih apakah bahan berlaku untuk Semua Varian (Base) atau hanya Varian Tertentu. Total harga dihitung dari harga pokok bahan.</p>
                <div id="recipes-container" class="space-y-3">
                    @forelse($product->recipes as $i => $recipe)
                    <div class="grid grid-cols-1 md:grid-cols-[1.2fr_2fr_1.2fr_1.2fr_auto] gap-3 recipe-row">
                        <select name="recipes[{{ $i }}][product_variant_id]" class="py-2.5 px-4 rounded-xl border border-outline-variant text-body-sm">
                            <option value="">Semua Varian</option>
                            @foreach($product->variants as $variant)
                            <option value="{{ $variant->id }}" {{ $recipe->product_variant_id == $variant->id ? 'selected' : '' }}>Khusus: {{ $variant->name }}</option>
                            @endforeach
                        </select>
                        <select name="recipes[{{ $i }}][ingredient_id]" class="py-2.5 px-4 rounded-xl border border-outline-variant text-body-sm" onchange="updateRecipeRow(this.closest('.recipe-row'))">
                            <option value="">Pilih bahan</option>
                            @foreach($ingredients as $ingredient)
                            <option value="{{ $ingredient->id }}" data-unit="{{ $ingredient->unit }}" data-cost-per-base="{{ $ingredient->cost_per_base_unit }}" data-base-qty="1" data-current-qty="{{ $ingredient->current_qty }}" data-track-stock="{{ $ingredient->track_stock ? '1' : '0' }}" {{ $recipe->ingredient_id == $ingredient->id ? 'selected' : '' }}>{{ $ingredient->name }} ({{ $ingredient->unit }})</option>
                            @endforeach
                        </select>
                        <div class="relative">
                            <input type="number" step="0.01" name="recipes[{{ $i }}][quantity]" value="{{ $recipe->quantity }}" placeholder="Qty" class="w-full py-2.5 pl-4 pr-14 rounded-xl border border-outline-variant text-body-sm" oninput="updateRecipeRow(this.closest('.recipe-row'))">
                            <span class="recipe-unit absolute right-4 top-1/2 -translate-y-1/2 text-label-sm font-semibold text-on-surface-variant">{{ $recipe->ingredient->unit ?? 'unit' }}</span>
                        </div>
                    ...
                    @empty
                    <div class="grid grid-cols-1 md:grid-cols-[1.2fr_2fr_1.2fr_1.2fr_auto] gap-3 recipe-row">
                        <select name="recipes[0][product_variant_id]" class="py-2.5 px-4 rounded-xl border border-outline-variant text-body-sm">
                            <option value="">Semua Varian</option>
                            @foreach($product->variants as $variant)
                            <option value="{{ $variant->id }}">Khusus: {{ $variant->name }}</option>
                            @endforeach
                        </select>
            ...
            <div class="flex items-center gap-3">
                <button type="submit" class="px-8 py-3 bg-primary text-on-primary rounded-xl font-semibold text-body-sm hover:bg-primary-container transition-colors min-h-[48px]">Update Produk</button>
    ...
    <script>
        ...
        let recipeIndex = {{ max($product->recipes->count(), 1) }};
        function addVariant() {
            const container = document.getElementById('variants-container');
            const row = document.createElement('div');
            row.className = 'flex items-center gap-3 variant-row';
            row.innerHTML = `<input type="text" name="variants[${variantIndex}][name]" placeholder="Nama varian" class="flex-1 py-2.5 px-4 rounded-xl border border-outline-variant text-body-sm"><input type="number" name="variants[${variantIndex}][price_modifier]" placeholder="Tambahan harga" value="0" class="w-40 py-2.5 px-4 rounded-xl border border-outline-variant text-body-sm"><button type="button" onclick="this.parentElement.remove()" class="p-2 text-danger hover:bg-red-50 rounded-lg"><span class="material-symbols-outlined text-[18px]">close</span></button>`;
        ...
        function variantOptions() {
            return `<option value="">Semua Varian</option>@foreach($product->variants as $variant)<option value="{{ $variant->id }}">Khusus: {{ $variant->name }}</option>@endforeach`;
        }
        ...
        function addRecipe() {
            ...
            const row = document.createElement('div');
            row.className = 'grid grid-cols-1 md:grid-cols-[1.2fr_2fr_1.2fr_1.2fr_auto] gap-3 recipe-row';
            row.innerHTML = `
                <select name="recipes[${recipeIndex}][product_variant_id]" class="py-2.5 px-4 rounded-xl border border-outline-variant text-body-sm">
                    ${variantOptions()}
                </select>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            document.querySelectorAll('.recipe-row').forEach(updateRecipeRow);
        });
    </script>
    @endpush
</x-layouts.admin>
